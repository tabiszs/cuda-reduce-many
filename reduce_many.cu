...
__device__ unsigned int getNoEndValue(unsigned int currArrayIdx, 
...
__device__ void warpReduce(volatile int *shm, int tid, int nEndValue) {
...
__global__ void reduce_many(double *g_idata, double *g_odata, 
    ...
    unsigned int nEndValue = getNoEndValue(currArrayIdx, nElementsInVector, nMemoryBlocksPerArray, currMemoryBlockIdx);
    ...
    if (tid < 32)
        warpReduce(sdata, tid, nEndValue);
...
__global__ void reduce_one(double *g_idata, double *g_odata, 
	...
	unsigned int idxInGlobalMemory = uniqueKernelID - (shift*currArrayIdx);
    unsigned int endValue = getEndValue(currArrayIdx, nElementsInVector, nblocksPerArray, blockIdx.x);
...
int local_reductions_many(int n, int m, double *d_in, double *d_out) 
{
    ...
    else
    {
        ...
        // Do first reduction
        reduce_many<nthreads><<<nblocksTotal, nthreads, shm_size>>>(
            d_in, d_out, m, optimalized_nblocksPerArray, nblocksPerArray);
        error = cudaGetLastError();
        if(error != cudaSuccess) 
            return error;
        //std::cout << optimalized_nblocksPerArray << std::endl;
